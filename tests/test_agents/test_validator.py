...
def invalid_qa_missing_option():
    return {
        "question": "What is required for aircraft maintenance?",
        "options": {"A": "Follow the approved schedule.", "B": "Ignore the schedule.", "C": "Ask a passenger."},  # Only 3 options
        "correct_answer": "A",
        "explanations": {"A": "This is the correct action.", "B": "Ignoring the schedule is unsafe.", "C": "Passengers are not responsible."},
        "source_reference": "Article 2",
        "difficulty": "easy",
        "focus": "conceptual"
    }
...
def test_validate_valid_qa(valid_qa, sample_chunk):
    ...
    mock_response.choices = [MagicMock(message=MagicMock(content='{"valid": true, "issues": [], "warnings": [], "checks_passed": {"has_4_options": true, "has_correct_answer": true, "has_all_explanations": true, "explanations_concise": true, "question_clear": true, "correct_explanation": true, "wrong_explanations_are_hints": true, "options_plausible": true, "question_unambiguous": true, "regulation_based": true}, "score": 10}'))]
...
def test_validate_batch(valid_qa, invalid_qa_missing_option, sample_chunk):
    ...
    with patch.object(Validator, 'validate', side_effect=[{"valid": True, "score": 10}, {"valid": False, "score": 7}]):
        validator = Validator(api_key="test-key")
        results = validator.validate_batch([valid_qa, invalid_qa_missing_option], sample_chunk)
